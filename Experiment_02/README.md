...
The circuit was simulated using Logisim Evolution, where different combinations of binary inputs were applied to verify the correctness of the outputs. The propagation of carry through each stage was observed.
...
3. Design a single full adder using XOR, AND, and OR gates.
4. Connect four full adders in series.
5. Connect carry output of each stage to carry input of the next stage.
6. Provide two 4-bit binary inputs.
7. Apply different input combinations.
8. Observe the sum and carry outputs.
9. Analyze the propagation delay during carry generation.

---

# Working Principle

The Ripple Carry Adder works by adding corresponding bits of two binary numbers along with the carry generated from the previous stage.

For each full adder:
- Sum is generated using XOR logic.
- Carry is generated using AND and OR logic.

The carry generated in one stage “ripples” to the next stage, which is why the circuit is called a Ripple Carry Adder.
...
- This sequential carry propagation makes the circuit slower compared to advanced adders such as Carry Look Ahead Adders.
